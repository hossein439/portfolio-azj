...
const getSingle = async () => {
    ...
    const { name, link, image, alt, gif, categories, category_id } = data.data[0];
    ...
    exFilenameImg.value = image;
    exFilenameGif.value = gif;

    imgSrc.value = setImageUrl(image);
...
const edit = async() => {
    ...
    formData.append('exFilenameImg', exFilenameImg.value);
    formData.append('exFilenameGif', exFilenameGif.value);
    formData.append('isChangedImg', isChangedImg.value);
    formData.append('isChangedGif', isChangedGif.value);


    await axios.put(`http://localhost:4000/effect/${route.params.id}`, formData)
    ...
    <form @submit.prevent="edit()">
        <div class="grid grid-cols-2 gap-5">
            ...
            <div :class="!imgSrc ? 'p-12' : 'p-3'"
                ...
                <input @change="selectImage($event, 'img')" class="absolute inset-0 opacity-0 cursor-pointer"
            ...
            <div :class="!gifSrc ? 'p-12' : 'p-3'"
                ...
                <input @change="selectImage($event, 'gif')" class="absolute inset-0 opacity-0 cursor-pointer"
                    type="file" accept=".gif">
